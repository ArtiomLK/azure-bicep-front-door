...
var privateLinkOriginDetails = [for i in range(0, length(pe_res_ids)) : {
  privateLink: {
    id: (pe_res_ids[i] == '') ? null : pe_res_ids[i]
  }
  groupId: (pl_res_types[i] == '') ? null : pl_res_types[i]
  privateLinkLocation: (pe_l[i] == '') ? null : pe_l[i]
...
resource origins 'Microsoft.Cdn/profiles/originGroups/origins@2021-06-01' = [ for i in range(0, length(origin_host_names)) : {
  ...
  properties: {
    ...
    priority: 1
    weight: 1000
    sharedPrivateLinkResource: empty(pe_res_ids) ? null : (pe_res_ids[i] == '')  ? null : privateLinkOriginDetails[i]
  }
}]
